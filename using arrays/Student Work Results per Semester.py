...
x = 0
while True:
    # мысль ... создать внутри цикла короткий список и добавлять в основной массив его вложенным массивом
    x = input()
    if x == str('#'):
        break
    B = x.split()
    print(B)
# со словарем не получится, т.к. словарь затирает предыдущие значения этого ключа
    N = {B[0]: B[1] for a in B}
    M.update(N)
